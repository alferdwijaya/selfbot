...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + " Kamu di kick ya? Kasihan\n(*´･ω･*)")
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + " Dadah Jangan kangen lagi sama Aku\n(*´･ω･*)")
    except Exception as e:
        print e
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                if msg.text == "gift":
                    sendMessage(msg.to, text="gift sent", contentMetadata=None, contentType=9)
                if msg.text == "set":
                    sendMessage(msg.to, "Aku akan mengirim daftar sider di grub ini denga ketik♪\n「tes」kalian akan terdeteksi♪")
                    try:
                ...
                if msg.text == "tes":
                    if msg.to in wait['readPoint']:
                        ...
                        sendMessage(msg.to, "Daftar mereka yang sider dan muncul di grub%s\nthat's it\n\nDaftar orang yang hanya sider dan tidak muncul di grub\n%sIt is abnormal ♪\n\nTanggal dan waktu pembuatan titik baca n time:\n[%s]"  % (wait['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                        sendMessage(msg.to, "Titik baca belum di tetapkan, silahlan ketik .\n「set」maka para sider terdeteksi")
                else:
                    pass
        else:
            pass
